  ...
  while( bcl >= p_param->bcl_end ){ // start BCL loop
    ...
    while( iout <= imax ){ // begin pacing loop
      ...
      if(iout % print_freq == 0){
        fprintf(fp_vm, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", tcurr, p_cell->STATES[V],p_cell->ALGEBRAIC[i_K1],p_cell->ALGEBRAIC[i_to],p_cell->ALGEBRAIC[i_Kr],p_cell->ALGEBRAIC[i_Ks],p_cell->ALGEBRAIC[i_CaL],p_cell->[i_NaK],p_cell->ALGEBRAIC[i_Na],p_cell->ALGEBRAIC[i_b_Na],p_cell->ALGEBRAIC[i_NaCa],p_cell->ALGEBRAIC[i_b_Ca],p_cell->ALGEBRAIC[i_p_K],p_cell->ALGEBRAIC[i_p_Ca]);
