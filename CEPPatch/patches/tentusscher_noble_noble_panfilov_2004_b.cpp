 ...
 * CONSTANTS[gamma] is gamma in component sodium_calcium_exchanger_current (dimensionless).
...
#endif
#if defined NACA_TN2004 
ALGEBRAIC[i_NaCa] = ( CONSTANTS[K_NaCa]*( exp(( CONSTANTS[gamma]*STATES[V]*CONSTANTS[F])/( CONSTANTS[R]*CONSTANTS[T]))*pow(STATES[Na_i], 3.00000)*CONSTANTS[Ca_o] -  exp(( (CONSTANTS[gamma] - 1.00000)*STATES[V]*CONSTANTS[F])/( CONSTANTS[R]*CONSTANTS[T]))*pow(CONSTANTS[Na_o], 3.00000)*STATES[Ca_i]*CONSTANTS[alpha]))/( (pow(CONSTANTS[Km_Nai], 3.00000)+pow(CONSTANTS[Na_o], 3.00000))*(CONSTANTS[Km_Ca]+CONSTANTS[Ca_o])*(1.00000+ CONSTANTS[K_sat]*exp(( (CONSTANTS[gamma] - 1.00000)*STATES[V]*CONSTANTS[F])/( CONSTANTS[R]*CONSTANTS[T]))));
