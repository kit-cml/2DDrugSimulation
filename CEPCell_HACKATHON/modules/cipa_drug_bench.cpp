...
void cipa_drug_bench( int argc, char **argv, param_t *p_param, bool is_dutta )
{
  ...
  FILE *fp_vm = NULL, * fp_ca = NULL, * fp_ires = NULL, *fp_maxdvdt_repol;
  ...
  assert( (p_param->num_pace1 > 250 && p_param->num_pace1 <= 2000) && "Pace should be more than 250 and less than 2000");
  ...
  // begin sample loop
  // THIS IS THE CORE PARALLEL LOOP THAT WE WANT TO IMPROVE USING GPU!!!
  ...
  { 
    ...
    { // begin concs loop
      ...
      if( p_param->is_print_graph == 1 ){
        ...
        snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_max_dvdt_repol_smp%d.plt", concs[idx], p_param->drug_name, concs[idx], sample_id );
        fp_maxdvdt_repol = fopen( buffer, "w" );
      }
      ...
      while(iout < imax){ // begin paces loop
        ...
        if( iout > 0 && iout % (int)( p_cell->CONSTANTS[BCL] / p_param->dt ) == 0 ) {
          ...
          if( is_eligible_AP && pace >= p_param->num_pace1-250) {
            ...
            fprintf( fp_maxdvdt_repol, "%d %lf\n", pace, temp_result.dvmdt_repol );

            // replace result with steeper repolarization AP among the last 250 paces
