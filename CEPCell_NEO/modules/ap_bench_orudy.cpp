...
void ap_bench_orudy( int argc, char **argv, param_t *p_param )
{
  // I/O variables
  int print_freq;
  char buff[255];
  ...
  if( (int)floor(p_param->celltype) == 0 ){
    printf("Using ORudy Endo.\n");
  }
  else if( (int)floor(p_param->celltype) == 1 ){
    printf("Using ORudy Epi.\n");
  }
  else if( (int)floor(p_param->celltype) == 2 ){
    printf("Using ORudy M.\n");
  }
