 ...
 * ALGEBRAIC[40] is gmma_d in component L_type_Ca_current_d_gate (millisecond).
 ...
 * CONSTANTS[27] is gmma in component sodium_calcium_exchanger_current (dimensionless).
...
ten_tusscher_model_2006_IK1Ko_endo_units::ten_tusscher_model_2006_IK1Ko_endo_units()
{
algebraic_size = 70;
constants_size = 53;
states_size = 19;
...
void ten_tusscher_model_2006_IK1Ko_endo_units::initConsts( )
{
STATES[0] = -86.709;
...
void ten_tusscher_model_2006_IK1Ko_endo_units::computeRates( double TIME, double *CONSTANTS, double *RATES, double *STATES, double *ALGEBRAIC )
{
...
ALGEBRAIC[17] = (STATES[0]<- 40.0000 ?  0.0570000*exp(- (STATES[0]+80.0000)/6.80000) : 0.000000);
...
ALGEBRAIC[18] = (STATES[0]<- 40.0000 ? (( ( - 25428.0*exp( 0.244400*STATES[0]) -  6.94800e-06*exp( - 0.0439100*STATES[0]))*(STATES[0]+37.7800))/1.00000)/(1.00000+exp( 0.311000*(STATES[0]+79.2300))) : 0.000000);
...
ALGEBRAIC[12] = (TIME -  floor(TIME/CONSTANTS[6])*CONSTANTS[6]>=CONSTANTS[5]&&TIME -  floor(TIME/CONSTANTS[6])*CONSTANTS[6]<=CONSTANTS[5]+CONSTANTS[7] ? - CONSTANTS[8] : 0.000000);
#else
if(isS1) ALGEBRAIC[12] = - CONSTANTS[8];
else ALGEBRAIC[12] = 0.0;
if(isEctopic) ALGEBRAIC[12] = - CONSTANTS[8];
else {
  if(!isS1) ALGEBRAIC[12] = 0.0;
}
#endif
...
void ten_tusscher_model_2006_IK1Ko_endo_units::solveAnalytical(double dt)
{
// Exact solution
