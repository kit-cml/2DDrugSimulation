...
void A1656D::mutate(double* ALGEBRAIC, double* STATES, double* CONSTANTS)
{
  ...
  double taum, tauh, tauj, minf, hinf, jinf;
...
*/
  taum = wt[4] / (exp(wt[5] * (STATES[V] - wt[3]) / RT) + exp(-wt[6] * (STATES[V] - wt[3]) / RT)) + wt[7];
  minf = pow( 1.0 / (1.0 + exp((STATES[V]-wt[1]) / wt[2])), 1.0/3.0 );
  tauh = wt[11] / (exp(wt[12] * (STATES[V] - wt[10]) / RT) + exp(-wt[13] * (STATES[V] - wt[10]) / RT)) + wt[14];
  hinf = 1.0 / (1.0 + exp((STATES[V] - wt[8]) / wt[9]));
  tauj = wt[16] / (exp(wt[17] * (STATES[V] - wt[15]) / RT) + exp(-wt[18] * (STATES[V] - wt[15]) / RT)) + wt[19];
  jinf = hinf;
/*  
...
*/
  ALGEBRAIC[alpha_m] = STATES[m]/taum;
  ALGEBRAIC[beta_m] = (1-STATES[m])/taum;
  ALGEBRAIC[alpha_h] = STATES[h]/tauh;
  ALGEBRAIC[beta_h] = (1-STATES[h])/tauh;
  ALGEBRAIC[alpha_j] = STATES[j]/tauj;
  ALGEBRAIC[beta_j] = (1-STATES[j])/tauj;

  ALGEBRAIC[tau_m] = ALGEBRAIC[alpha_m] * ALGEBRAIC[beta_m];
  ALGEBRAIC[tau_h] = 1/(ALGEBRAIC[alpha_h] + ALGEBRAIC[beta_h]);
  ALGEBRAIC[tau_j] = 1/(ALGEBRAIC[alpha_j] + ALGEBRAIC[beta_j]);


  ALGEBRAIC[i_Na] = CONSTANTS[Cm]*
                    wt[0]*
                    pow(STATES[m], 3.0)*
                    ((wt[20]*STATES[h]) + ((1.0-wt[20])*STATES[j]))*
