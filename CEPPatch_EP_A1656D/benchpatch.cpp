...
int main( int argc, char **argv )
{
  ...
  while( iout <= imax ){
    ...
    if(iout % print_freq == 0){
      fprintf(fp_curr, "%lf %lf %lf %lf %lf %lf %lf\n",
               tcurr,p_patch->ALGEBRAIC[i_Na],p_patch->ALGEBRAIC[i_CaL],p_patch->ALGEBRAIC[i_to],p_patch->ALGEBRAIC[i_Kr],p_patch->ALGEBRAIC[i_Ks],p_patch->ALGEBRAIC[i_K1]);
      fprintf(fp_vm, "%lf %lf\n", tcurr, p_patch->STATES[V]);
    }
    if( pace > 10 ){
      if( tcurr >= ( (pace-10) * p_patch->CONSTANTS[stim_period] ) && iout % print_freq == 0 ){
  ...
  for( i = 0; i < p_patch->states_size; i++ ){
