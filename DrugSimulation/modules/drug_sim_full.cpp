...
Cellmodel *p_cell, cvode_t *p_cvode, qinward_t *p_qin, bool is_firsttime)
{
  ...
  if( is_print_graph == true ){
    snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_vmcheck_optimal.plt", 
              conc, drug_name, conc );
    fp_vm = fopen( buffer, "w" );
    snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_dvmdt_optimal.plt", 
              conc, drug_name, conc);
    fp_dvmdt = fopen( buffer, "w" );
    snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_ca_i_optimal.plt", 
              conc, drug_name, conc );
    fp_ca = fopen( buffer, "w" );
    snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_ires_optimal.plt", 
              conc, drug_name, conc );
    fp_ires = fopen( buffer, "w" );
    snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_inet_optimal.plt", 
              conc, drug_name, conc );
  ...
  snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_feature_optimal.plt", conc, p_param->drug_name, conc );
  fp_feature = fopen( buffer, "w" );
  snprintf(buffer, sizeof(buffer), "result/%.2lf/%s_%.2lf_output_optimal.plt", conc, p_param->drug_name, conc );
  ...
  // begin simulation loop
  {
    ...
    if( cvode_retval == CV_SUCCESS ){
      ...
      mpi_fprintf(0, stderr, "CVode error at sample_ID %d and concentration %.2lf at rank %d\n", 
    ...
    if(icount % (int)(bcl/dt) == 0 && icount > 0){
      ...
      if( (int)ceil(conc) == 0 ) {
        ...
        fprintf( fp_feature, "%d %lf %lf\n", pace_count, cipa_result.qnet, 0.0 );
      }
      else{
        ...
        fprintf( fp_feature, "%d %lf %lf\n", pace_count, cipa_result.qnet, qinward );
      }
