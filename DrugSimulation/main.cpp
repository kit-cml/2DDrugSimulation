...
int main(int argc, char **argv)
{
  ...
  else if( p_param->simulation_mode > 0 && ic50.size() > 1 ){
    mpi_printf(0, "Make sure only use the optimal sample for full-pace simulation!\n");
    MPI_Finalize();
  }
  ...
  if(mympi::rank == 0){
    mkdir("result/", 0775);
    snprintf( buffer, sizeof(buffer), "result/0.00" );
  ...
  // broadcast qin data from node 0 to other nodes
  ...
  if( p_param->simulation_mode == 0 )
  {
    ...
    { // begin sample loop
      ...
      { // begin concentration loop
        ...
        if(is_ead == true){
          ead_counter++;
          printf("EAD happened in drug %s sample %hu concentration %.2lf\n", p_param->drug_name, sample_id, concs[idx]);
        }
  ...
  if( mympi::rank == 0 ){
    ...
    snprintf( buffer, sizeof(buffer), "result/performance_%s.log", buff_time );
    fp_perf_log = fopen( buffer, "w" );
    fprintf( fp_perf_log, "Iteration BCL Timestep CPU Sample Pacing_Time(Minutes) Total_EAD_Cases\n" );
    fprintf( fp_perf_log, "%lld %.2lf %.2lf %d %lu %d %lf %hu\n", (long long int)(ic50.size()*concs.size()*p_param->pace_max*p_cell->CONSTANTS[BCL]/p_param->dt), p_cell->CONSTANTS[BCL], p_param->dt, mympi::size, ic50.size(), p_param->pace_max, (t_end-t_begin)/60., ead_total );
