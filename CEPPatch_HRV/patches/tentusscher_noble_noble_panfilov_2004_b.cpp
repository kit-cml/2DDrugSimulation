...
for(int i = 0; i < algebraic_size; i++){ALGEBRAIC[i] = 0.;}
for(int i = 0; i < constants_size; i++){CONSTANTS[i] = 0.;}
for(int i = 0; i < states_size; i++){STATES[i] = 0.;}
}
...
//ALGEBRAIC[i_Stim] =  (TIME >= CONSTANTS[stim_start] && TIME <= CONSTANTS[stim_start]+CONSTANTS[stim_duration]) || (TIME >= CONSTANTS[stim_start]+CONSTANTS[last_stim_period] && TIME <= CONSTANTS[stim_start]+CONSTANTS[last_stim_period]+CONSTANTS[stim_duration]) ? - CONSTANTS[stim_amplitude] : 0.000000;
if( ALGEBRAIC[i_Stim] != 0 ){
  printf("STIMULATED AT %lf msec\n", TIME);
}
#elif defined TISSUE
if(isS1) ALGEBRAIC[i_Stim] = - CONSTANTS[stim_amplitude];
