...
ALGEBRAIC[Istim] =  (TIME >= CONSTANTS[stim_start] && TIME <= CONSTANTS[stim_start]+CONSTANTS[duration]) || (TIME >= CONSTANTS[stim_start]+CONSTANTS[last_stim_period] && TIME <= CONSTANTS[stim_start]+CONSTANTS[last_stim_period]+CONSTANTS[duration]) ? CONSTANTS[amp] : 0.000000;
#endif
#if defined CAB_ORUDY2011 
